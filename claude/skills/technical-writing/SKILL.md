...
description: "Layered technical-writing standard: Diátaxis structure, Google developer style sentences, STE instruction rules, Global English syntax. Explicit invocation only. Covers document shape and sentence load; ~/.claude/writing-style.md covers voice and wins on any conflict. Use for /technical-writing or when writing or reviewing docs, RFCs, readmes, or specs."
...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Add new offenders to check 10 in `~/.claude/writing-style.md` with their replacement.
...
- Run the tier 2 pass in `~/.claude/writing-style.md` over every doc this skill touches. That file owns the tell catalog: AI vocabulary, filler, hedging, formatting tells. For a doc that needs stripping rather than checking, invoke the **unslop** skill first, then tier 2.
- PR descriptions and commit messages are writing too. Every layer except Diátaxis applies to them, on top of the tier 1 rules that already cover them.
