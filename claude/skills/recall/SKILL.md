...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the workspace path with each "/" turned into "-", the leading one included (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message. The same directory holds `memory/`, the auto-memory files indexed by `memory/MEMORY.md`; read that index first, since it is a curated record of what mattered and it is cheap.

1. Classify, then route. Resuming one specific prior chat is `claude --resume`, not this. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief under the tier 1 rules in `~/.claude/writing-style.md`, cite chat findings by UUID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
