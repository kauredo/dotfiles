# Writing style reference

The catalogue behind check 14 of the mandatory final pass in `~/.claude/writing-style.md`.
Not always-loaded. Check 14 opens this file and greps the draft against every entry.

## Kill these patterns (highest-signal AI tells)

- **Rule-of-three saturation.** Stop reaching for three parallel items, three adjectives, three clauses. One or two is usually truer.
- **Em dashes.** None. Not spaced, not unspaced, not as an aside, not "an occasional one". If a thought needs separating, end the sentence or use a comma. Reaching for a parenthesis instead trades one tell for another, so prefer the full stop.
- **False ranges.** "from X to Y" where X and Y aren't ends of a real scale ("from onboarding to billing") is a list wearing a range's clothes. Just list the items.
- **Significance-inflation verbs.** Use plain copulas. Not "serves as / stands as / underscores / represents / is a testament to / exemplifies / embodies / reflects / highlights / showcases". Just "is / has / does".
- **Present-participle summary tails.** Don't end sentences with "…, highlighting the broader implications" / "…, underscoring the importance of X". They genuflect at significance and add nothing.
- **Hedge openers / throat-clearing.** No "In today's fast-moving landscape," "When it comes to X," "As we navigate." Start with the actual point.
- **Resolution/summary closers.** No "Overall," "In conclusion," "At the end of the day," and no one-line section recap that restates what was just said.
- **Sycophantic openers.** No "Great question," "What a fascinating idea."
- **"Reads" as an evaluation verb.** No "the fix reads right," "this reads clean," "it reads as X." Say "the fix looks right," or just state the judgment plainly ("the logic is correct"). Bare **"holds"** is the same crutch as "holds up": no "your reading holds," "the claim holds," "the invariant holds." Say what was checked and what came back, or state the judgment.
- **Recapping what's already settled.** When acknowledging that something is done or resolved, don't list each resolved item back to the person. They did the work, they know what it was. Say it's handled and move on. Kill the "both threads resolved: X, and Y" outline.

## Structure and rhythm

- **Repeat nouns; don't do elegant variation.** Call the thing the same name each time. Don't cycle "the tool / the app / the platform / the solution" for one product.
- **Prose over bullet-list-itis.** Don't convert narrative into bulleted lists with bold inline headers when sentences would read better. The specific tell is a bold label plus colon that restates the line after it ("**Performance:** Performance improved by…"). A bold lead-in that ends in a period, names the thing, and is followed by genuinely new detail ("**Schema in TypeScript.** Tables live in one file.") is fine and not a tell, so don't over-correct into removing every bold lead-in.
- **No vague attribution.** Skip "experts argue," "studies show," "industry reports" unless you have a real, named source.
- **Sentence case headings, straight quotes.** Not Title Case On Every Heading, and no curly quotes or decorative emoji in headings and bullets.
