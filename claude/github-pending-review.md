...
**Hard gate, no exceptions.** Before ANY of that text is posted or shown to me for approval, run the mandatory final pass in `~/.claude/writing-style.md` ("Mandatory final pass"). All 10 checks, structural ones first, phrase grep last. Read that file if it is not already in context. This applies to a one-line reply in an existing thread exactly as much as to a full review, and it applies again on every follow-up round: a draft that was fine last round is not exempt, and reusing last round's sentence shapes is itself one of the checks.
